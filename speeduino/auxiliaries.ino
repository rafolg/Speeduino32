...
void initialiseFan(void)
{
  ...
  currentStatus.fanDuty = 0;
    ...
    if ( configPage2.fanEnable == 2 ) // PWM Fan control
    {
        ...
        fan_pwm_max_count = 1000000L / (32 * configPage6.fanFreq * 2); //Converts the frequency in Hz to the number of ticks (at 32uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 512hz
      #elif defined(CORE_TEENSY41)
        fan_pwm_max_count = 1000000L / ((8533 * configPage6.fanFreq * 2) / 10000); //Converts the frequency in Hz to the number of ticks (at 2uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 512hz
      #endif
      fan_pwm_value = 0;
    }
  #endif
...
void initialiseAuxPWM(void)
{
  ...
  wmi_pin_port = portOutputRegister(digitalPinToPort(pinWMI));
  wmi_pin_mask = digitalPinToBitMask(pinWMI);
#endif
  ...
  if( configPage6.vvtEnabled > 0)
  {
    ...
    #elif defined(CORE_TEENSY41)
      vvt_pwm_max_count = 1000000L / ((8533 * configPage6.vvtFreq * 2) / 10000); //Converts the frequency in Hz to the number of ticks (at 2uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming fro TS to allow for up to 512hz
    #endif
  ...
  {
#if defined(CORE_TEENSY35)
    wmi_pwm_max_count = 1000000L / (32 * configPage6.wmiFreq * 2); //**** NOT YET IMPLEMENTED ****Converts the frequency in Hz to the number of ticks (at 16uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 512hz
#elif defined(CORE_TEENSY41)
    wmi_pwm_max_count = 1000000L / ((8533 * configPage10.wmiFreq * 2) / 10000);
#endif
    BIT_CLEAR(currentStatus.status4, BIT_STATUS4_WMI_EMPTY);
    currentStatus.wmiPW = 0;
    wmi_pwm_target_value = 0;
    ENABLE_WMI_TIMER();
  }
#endif
...
void wmiControl(void)
{
  ...
  // wmi can only work when vvt is disabled in CORE_AVR
#if defined(CORE_AVR)
  if( (configPage6.vvtEnabled == 0) && (configPage10.wmiEnabled >= 1) )
#elif defined(CORE_TEENSY)
  if(configPage10.wmiEnabled >=1)
...
#elif defined(CORE_TEENSY)
    wmi_pwm_target_value = percentage(currentStatus.wmiPW, wmi_pwm_max_count);
#endif
...
#endif
{
  if (boost_pwm_state == true)
  {
    ...
    //#endif
    COMPARE_BOOST_UPDATE_ENA(); //FlexTimer implementation
    SET_COMPARE(BOOST_TIMER_COMPARE, BOOST_TIMER_COUNTER + (boost_pwm_max_count - boost_pwm_cur_value) );
    COMPARE_BOOST_DISABLE_DIS(); //FlexTimer implementation
    //DISABLE_BOOST_TIMER(); //PIT timer fix
    //ENABLE_BOOST_TIMER(); //PIT timer fix
  ...
  else
  {
    ...
    COMPARE_BOOST_DISABLE_DIS(); //FlexTimer implementation
    //DISABLE_BOOST_TIMER(); //PIT timerfix
    //ENABLE_BOOST_TIMER(); //PIT timer fix
...
#endif
{
  if ( ((vvt1_pwm_state == false) || (vvt1_max_pwm == true)) && ((vvt2_pwm_state == false) || (vvt2_max_pwm == true)) )
  {
    ...
    {
      COMPARE_VVT_UPDATE_ENA(); //FlexTimer implementation
      SET_COMPARE(VVT_TIMER_COMPARE, VVT_TIMER_COUNTER + vvt1_pwm_value);
      COMPARE_VVT_DISABLE_DIS(); //FlexTimer implementation
      //DISABLE_VVT_TIMER(); //PIT timer fix
      //ENABLE_VVT_TIMER(); //PIT timer fix
    ...
    else { COMPARE_VVT_UPDATE_ENA(); SET_COMPARE(VVT_TIMER_COMPARE, VVT_TIMER_COUNTER + vvt_pwm_max_count); COMPARE_VVT_DISABLE_DIS(); } //Shouldn't ever get here
  }
  else
  {
    if(nextVVT == 0)
    {
      ...
      nextVVT = 1; //Next event is for PWM1
      if(vvt2_pwm_state == true){ COMPARE_VVT_UPDATE_ENA(); SET_COMPARE(VVT_TIMER_COMPARE, VVT_TIMER_COUNTER + (vvt2_pwm_cur_value - vvt1_pwm_cur_value) ); COMPARE_VVT_DISABLE_DIS(); }
      else
      { 
        COMPARE_VVT_UPDATE_ENA(); //FlexTimer implementation
    ...
    else if (nextVVT == 1)
    {
      ...
      if(vvt1_pwm_state == true) { COMPARE_VVT_UPDATE_ENA(); SET_COMPARE(VVT_TIMER_COMPARE, VVT_TIMER_COUNTER + (vvt1_pwm_cur_value - vvt2_pwm_cur_value) ); COMPARE_VVT_DISABLE_DIS(); }
      else
      { 
        COMPARE_VVT_UPDATE_ENA(); //FlexTimer implementation
...
//The interrupt to control the WMI PWM -- Only for CORE_TEENSY
#if defined(CORE_TEENSY)
void wmiInterrupt(void) //Most ARM chips can simply call a function
{
  if (wmi_pwm_state == true)
  {
    WMI_PIN_LOW();  // Switch pin to low
    COMPARE_WMI_UPDATE_ENA();
    SET_COMPARE(WMI_TIMER_COMPARE, WMI_TIMER_COUNTER + (wmi_pwm_max_count - wmi_pwm_cur_value) );
    COMPARE_WMI_DISABLE_DIS();
    wmi_pwm_state = false;
  }
  else
  {
    WMI_PIN_HIGH();  // Switch pin high
    COMPARE_WMI_UPDATE_ENA();
    SET_COMPARE(WMI_TIMER_COMPARE, WMI_TIMER_COUNTER + wmi_pwm_target_value);
    COMPARE_WMI_DISABLE_DIS();
    wmi_pwm_cur_value = wmi_pwm_target_value;
    wmi_pwm_state = true;
  }
}
#endif
