...
bool knockOn = false; /**< The current state of the knock detection system (on or off) */
...
void loop(void)
{
      ...
      if (!BIT_CHECK(currentStatus.status4, BIT_STATUS4_WSER) && serialTransmitInProgress())
      {
         serialTransmit();
      }
      //Check for any new or in-progress requests from serial.
      if ((!BIT_CHECK(currentStatus.status4, BIT_STATUS4_WSER) && Serial.available()>0) || (!BIT_CHECK(currentStatus.status4, BIT_STATUS4_WSER) && serialRecieveInProgress()))
      {
         serialReceive();
      }
      //BTSERIAL Comms
      //Initially check that the last BTserial send values request is not still outstanding
      if (BIT_CHECK(currentStatus.status4, BIT_STATUS4_WSER) && serialTransmitInProgress())
      {
         BTserialTransmit();
      }
      //Check for any new or in-progress requests from BTserial.
      if ((BIT_CHECK(currentStatus.status4, BIT_STATUS4_WSER) && BTserial.available()>0) || (BIT_CHECK(currentStatus.status4, BIT_STATUS4_WSER) && serialRecieveInProgress()))
      {
         BTserialReceive();
      }
    ...
    else
    {
      ...
      fuelOn = false;
      knockOn = false;
      currentStatus.knock1 = 0;
      currentStatus.knock2 = 0;
      currentStatus.knock3 = 0;
      currentStatus.knock4 = 0;
      currentStatus.knock1A = 0;
      currentStatus.knock2A = 0;
      currentStatus.knock3A = 0;
      currentStatus.knock4A = 0;
      currentStatus.knock1B = 0;
      currentStatus.knock2B = 0;
      currentStatus.knock3B = 0;
      currentStatus.knock4B = 0;
      currentStatus.knock1C = 0;
      currentStatus.knock2C = 0;
      currentStatus.knock3C = 0;
      currentStatus.knock4C = 0;
      currentStatus.knock1D = 0;
      currentStatus.knock2D = 0;
      currentStatus.knock3D = 0;
      currentStatus.knock4D = 0;
      currentStatus.knockTC = SPU_SET_INTEGRATOR_TIME; BIT_CLEAR(currentStatus.KNOCK, BIT_KNOCK_TC);
    ...
    if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ)) //10 hertz
    {
      ...
      #if defined(NATIVE_CAN_AVAILABLE)
        if (configPage2.canHALTECHv2 == true) { sendHALTECHv210Hz(); }
      #endif
    ...
    if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_30HZ)) //30 hertz
    {
      ...
      if (configPage2.canHALTECHv2 == true) { sendHALTECHv230Hz(); }
      #endif
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_4HZ))
    {
      ...
      #if defined(NATIVE_CAN_AVAILABLE)
        if (configPage2.canHALTECHv2 == true) { sendHALTECHv24Hz(); }
      #endif
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_1HZ)) //Once per second)
    {
      ...
      if(knockOn)
      {
        currentStatus.knockFreqBinA = getFrequency(configPage15.knockFreqA); 
        currentStatus.knockFreqBinB = getFrequency(configPage15.knockFreqB); 
        currentStatus.knockFreqBinC = getFrequency(configPage15.knockFreqC); 
        currentStatus.knockFreqBinD = getFrequency(configPage15.knockFreqD); 
        
        uint8_t tempknockGainBin = getProgGain(configPage15.knockGain);
        if (tempknockGainBin != currentStatus.knockGainBin) { currentStatus.knockGainBin = tempknockGainBin; BIT_CLEAR(currentStatus.KNOCK, BIT_KNOCK_AP); }
        else { BIT_SET(currentStatus.KNOCK, BIT_KNOCK_AP); }
      }
      #ifdef SD_LOGGING
    ...
    if ((currentStatus.hasSync || BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC)) && (currentStatus.RPM > 0))
    {
        ...
        if(currentStatus.RPM > currentStatus.crankRPM) //Crank RPM in the config is stored as a x10. currentStatus.crankRPM is set in timers.ino and represents the true value
        {
          ...
          if(configPage15.knockEnable > 0) { knockOn = true; }
          else { knockOn = false; }
        }
        else
        {  
      ...
      if (knockCount <= 4)
        { BIT_CLEAR(currentStatus.KNOCK, BIT_KNOCK_HZA); }
      else { BIT_SET(currentStatus.KNOCK, BIT_KNOCK_HZA); BIT_CLEAR(currentStatus.KNOCK2, BIT_KNOCK2_VALA); }
      
      if ((knockCount >= 5) && (knockCount <= 8))
        { BIT_CLEAR(currentStatus.KNOCK, BIT_KNOCK_HZB); }
      else { BIT_SET(currentStatus.KNOCK, BIT_KNOCK_HZB); BIT_CLEAR(currentStatus.KNOCK2, BIT_KNOCK2_VALB); } 
        
      if ((knockCount >= 9) && (knockCount <= 12))
        { BIT_CLEAR(currentStatus.KNOCK, BIT_KNOCK_HZC); }
      else { BIT_SET(currentStatus.KNOCK, BIT_KNOCK_HZC); BIT_CLEAR(currentStatus.KNOCK2, BIT_KNOCK2_VALC); } 
      
      if ((knockCount >= 13) && (knockCount <= 16))
        { BIT_CLEAR(currentStatus.KNOCK, BIT_KNOCK_HZD); }
      else { BIT_SET(currentStatus.KNOCK, BIT_KNOCK_HZD); BIT_CLEAR(currentStatus.KNOCK2, BIT_KNOCK2_VALD); }
      ...
      if(!BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN) || (configPage6.vvtCLAlterFuelTiming < 1))
      {
        //If the user has elected to disable VVT fuel correcting during cranking, make sure it's off 
        currentStatus.injAngle = table2D_getValue(&injectorAngleTable, currentStatus.RPMdiv100);
      }
      else      
      {
        currentStatus.injAngle = table2D_getValue(&injectorAngleTable, currentStatus.RPMdiv100) - currentStatus.vvt1Angle;
      }
      ...
      injector1StartAngle = calculateInjectorStartAngle(PWdivTimerPerDegree, CRANK_ANGLE_MAX_INJ); /*channel1InjDegrees*/
      ...
      //***********************************************************************************************
      //| BEGIN IGNITION AND KNOCK CALCULATIONS
      ...
      //if( (configPage2.perToothIgn == true) && ((currentStatus.RPM < 1950) || (currentStatus.RPM > 2050)) ) { perToothIgn2 = true; triggerSetEndTeethK(); }
      //else if ( (configPage2.perToothIgn == true) && (currentStatus.RPM >= 1950) && (currentStatus.RPM <= 2050) ) { perToothIgn2 = false; }
      ...
      //while (crankAngleK > CRANK_ANGLE_MAX_KNK ) { crankAngleK -= CRANK_ANGLE_MAX_KNK; }
      ...
      int knkStart = table2D_getValue(&knockWindowStartTable, currentStatus.RPMdiv100) - OFFSET_KNOCK;
      int knkWindow = table2D_getValue(&knockWindowDurationTable, currentStatus.RPMdiv100);
      calculateKnockAngles(knkStart, knkWindow);
      currentStatus.knockWindowTime = angleToTime(knkWindow, CRANKMATH_METHOD_INTERVAL_REV);
      uint8_t tempknockTC = tcCalculator(currentStatus.knockWindowTime);
      if(tempknockTC != currentStatus.knockTC)
       {
        currentStatus.knockTC = tempknockTC; 
        BIT_CLEAR(currentStatus.KNOCK, BIT_KNOCK_TC);
       }
      else {BIT_SET(currentStatus.KNOCK, BIT_KNOCK_TC);}
      ...
      if(knockOn)
      {
        ...
        while (crankAngle > CRANK_ANGLE_MAX_IGN ) { crankAngle -= CRANK_ANGLE_MAX_IGN; }
            ...
            */
            ...
            unsigned long knock1StartTime = 0;
...
#endif
#if defined(USE_KNK_REFRESH)
        if( (knockSchedule1.Status == RUNNING) && (knock1EndAngle > crankAngleK) && (perToothIgn2 != true) )
        {
          unsigned long uSToEnd = 0;
          crankAngleK = getCrankAngle(); //Refresh with the latest crank angle
          if (crankAngleK > CRANK_ANGLE_MAX_KNK ) { crankAngleK -= 360; }
          if(knock1EndAngle > crankAngleK) { uSToEnd = fastDegreesToUS( (knock1EndAngle - crankAngleK) ); }
          else { uSToEnd = fastDegreesToUS( (360 + knock1EndAngle - crankAngleK) ); }
          refreshKnockSchedule1( uSToEnd );
        }
#endif
            ...
            */
            ...
            unsigned long knock2StartTime = 0;
...
#endif
#if defined(USE_KNK_REFRESH)
        if( (knockSchedule2.Status == RUNNING) && (knock2EndAngle > crankAngleK) && (perToothIgn2 != true) )
        {
          unsigned long uSToEnd = 0;
          crankAngleK = getCrankAngle(); //Refresh with the latest crank angle
          if (crankAngleK > CRANK_ANGLE_MAX_KNK ) { crankAngleK -= 360; }
          if(knock2EndAngle > crankAngleK) { uSToEnd = fastDegreesToUS( (knock2EndAngle - crankAngleK) ); }
          else { uSToEnd = fastDegreesToUS( (360 + knock2EndAngle - crankAngleK) ); }
          refreshKnockSchedule2( uSToEnd );
        }
#endif
            ...
            */
            ...
            unsigned long knock3StartTime = 0;
...
#endif
#if defined(USE_KNK_REFRESH)
        if( (knockSchedule3.Status == RUNNING) && (knock3EndAngle > crankAngleK) && (perToothIgn2 != true) )
        {
          unsigned long uSToEnd = 0;
          crankAngleK = getCrankAngle(); //Refresh with the latest crank angle
          if (crankAngleK > CRANK_ANGLE_MAX_KNK ) { crankAngleK -= 360; }
          if(knock3EndAngle > crankAngleK) { uSToEnd = fastDegreesToUS( (knock3EndAngle - crankAngleK) ); }
          else { uSToEnd = fastDegreesToUS( (360 + knock3EndAngle - crankAngleK) ); }
          refreshKnockSchedule3( uSToEnd );
        }
#endif
            ...
            */
            ...
            unsigned long knock4StartTime = 0;
...
#endif
#if defined(USE_KNK_REFRESH)
        if( (knockSchedule4.Status == RUNNING) && (knock4EndAngle > crankAngleK) && (perToothIgn2 != true) )
        {
          unsigned long uSToEnd = 0;
          crankAngleK = getCrankAngle(); //Refresh with the latest crank angle
          if (crankAngleK > CRANK_ANGLE_MAX_KNK ) { crankAngleK -= 360; }
          if(knock4EndAngle > crankAngleK) { uSToEnd = fastDegreesToUS( (knock4EndAngle - crankAngleK) ); }
          else { uSToEnd = fastDegreesToUS( (360 + knock4EndAngle - crankAngleK) ); }
          refreshKnockSchedule4( uSToEnd );
        }
#endif

      } //Knock schedules on
      else { currentStatus.knock1 = 0; currentStatus.knock2 = 0; currentStatus.knock3 = 0; currentStatus.knock4 = 0;
             currentStatus.knock1A = 0; currentStatus.knock2A = 0; currentStatus.knock3A = 0; currentStatus.knock4A = 0;
             currentStatus.knock1B = 0; currentStatus.knock2B = 0; currentStatus.knock3B = 0; currentStatus.knock4B = 0; 
             currentStatus.knock1C = 0; currentStatus.knock2C = 0; currentStatus.knock3C = 0; currentStatus.knock4C = 0;
             currentStatus.knock1D = 0; currentStatus.knock2D = 0; currentStatus.knock3D = 0; currentStatus.knock4D = 0; }
...
uint16_t calculateInjectorStartAngle(uint16_t PWdivTimerPerDegree, int16_t injChannelDegrees)
{
  ...
  if(tempInjectorStartAngle > (uint16_t)CRANK_ANGLE_MAX_INJ) { tempInjectorStartAngle -= CRANK_ANGLE_MAX_INJ; }
...
void calculateKnockAngles(int knkStart, int knkWindow)
{
  switch (configPage2.nCylinders)
  {
    //4 cylinders
    case 4:
     calculateKnockAngle1(knkStart, knkWindow);
     calculateKnockAngle2(knkStart, knkWindow);
     calculateKnockAngle3(knkStart, knkWindow);
     calculateKnockAngle4(knkStart, knkWindow);
     break;
