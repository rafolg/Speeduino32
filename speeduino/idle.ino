  ...
  switch(configPage6.iacAlgorithm)
  {
    ...
    case IAC_ALGORITHM_PWM_OL:
      ...
      iacCrankDutyTable.axisX = configPage6.iacCrankBins;
        ...
        idle_pwm_max_count = 1000000L / ((8533 * configPage6.idleFreq * 2)/10000); //Converts the frequency in Hz to the number of ticks (at 2uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 512hz
      #endif
      enableIdle();
      break;
      ...
      else
      {
        ...
        if(PID_computed == true)
        {
          ...
          if(configPage15.airConIdleSteps>0 && BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
          {
            ...
            if((unsigned long)TEMP_idle_pwm_target_value > (idle_pwm_max_count<<2)) { TEMP_idle_pwm_target_value = (idle_pwm_max_count<<2); }
          }
    ...
    case IAC_ALGORITHM_PWM_OLCL: //case 6 is PWM Open Loop table as feedforward term plus closed loop. 
      ...
      else
      {
        ...
        if(configPage15.airConIdleSteps>0 && BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
        {
          ...
          if((unsigned long)FeedForwardTerm > (idle_pwm_max_count<<2)) { FeedForwardTerm = (idle_pwm_max_count<<2); }
        }
...
#endif
{
  if (idle_pwm_state)
  {
    ...
    COMPARE_IDLE_UPDATE_ENA();
    SET_COMPARE(IDLE_COMPARE, IDLE_COUNTER + (idle_pwm_max_count - idle_pwm_cur_value) );
    COMPARE_IDLE_DISABLE_DIS();
    //IDLE_TIMER_DISABLE(); //PIT timer fix
    //IDLE_TIMER_ENABLE(); //PIT timer fix
