...
void BTlegacySerialCommand(void)
{
  if ( serialStatusFlag == SERIAL_INACTIVE ) { currentCommand = BTserial.read(); }
  ...
  switch (currentCommand)
  {
    ...
    case 'a':
      ...
      {
        BTserial.read(); //Ignore the first value, it's always 0
        BTserial.read(); //Ignore the second value, it's always 6
        BTsendValuesLegacy();
        serialStatusFlag = SERIAL_INACTIVE;
      }
      break;
    ...
    case 'A': // send x bytes of realtime values
      BTsendValues(0, LOG_ENTRY_SIZE, 0x31, 0);   //send values to serial0
      break;
    ...
    case 'b': // New EEPROM burn command to only burn a single page at a time
      ...
      if (BTserial.available() >= 2)
      {
        BTserial.read(); //Ignore the first table value, it's always 0
        writeConfig(BTserial.read());
        serialStatusFlag = SERIAL_INACTIVE;
      }
      break;
    ...
    case 'c': //Send the current loops/sec value
      BTserial.write(lowByte(currentStatus.loopsPerSecond));
      BTserial.write(highByte(currentStatus.loopsPerSecond));
      break;
    ...
    case 'd': // Send a CRC32 hash of a given page
      ...
      if (BTserial.available() >= 2)
      {
        BTserial.read(); //Ignore the first byte value, it's always 0
        uint32_t CRC32_val = calculatePageCRC32( BTserial.read() );
        
        //Split the 4 bytes of the CRC32 value into individual bytes and send
        BTserial.write( ((CRC32_val >> 24) & 255) );
        BTserial.write( ((CRC32_val >> 16) & 255) );
        BTserial.write( ((CRC32_val >> 8) & 255) );
        BTserial.write( (CRC32_val & 255) );
    ...
    case 'E': // receive command button commands
      ...
      if(BTserial.available() >= 2)
      {
        byte cmdGroup = (byte)BTserial.read();
        (void)TS_CommandButtonsHandler(word(cmdGroup, BTserial.read()));
        serialStatusFlag = SERIAL_INACTIVE;
      }
      break;

    case 'F': // send serial protocol version
      BTserial.print(F("001"));
      break;
    ...
    case 'G': // Dumps the EEPROM values to serial
      ...
      //The format is 2 bytes for the overall EEPROM size, a comma and then a raw dump of the EEPROM values
      BTserial.write(lowByte(getEEPROMSize()));
      BTserial.write(highByte(getEEPROMSize()));
      BTserial.print(',');

      for(uint16_t x = 0; x < getEEPROMSize(); x++)
      {
        BTserial.write(EEPROMReadRaw(x));
      }
      serialStatusFlag = SERIAL_INACTIVE;
      break;
    ...
    {
      //Format is similar to the above command. 2 bytes for the EEPROM size that is about to be transmitted, a comma and then a raw dump of the EEPROM values
      while(BTserial.available() < 3) { delay(1); }
      uint16_t eepromSize = word(BTserial.read(), BTserial.read());
      if(eepromSize != getEEPROMSize())
      {
        //Client is trying to send the wrong EEPROM size. Don't let it 
        BTserial.println(F("ERR; Incorrect EEPROM size"));
        break;
      }
      else
      {
        ...
        {
          while(BTserial.available() < 3) { delay(1); }
          EEPROMWriteRaw(x, BTserial.read());
        }
      }
      serialStatusFlag = SERIAL_INACTIVE;
      break;
    ...
    case 'H': //Start the tooth logger
      startToothLogger();
      BTserial.write(1); //TS needs an acknowledgement that this was received. I don't know if this is the correct response, but it seems to work
      break;
    ...
    case 'm': //Send the current free memory
      currentStatus.freeRAM = freeRam();
      BTserial.write(lowByte(currentStatus.freeRAM));
      BTserial.write(highByte(currentStatus.freeRAM));
      break;
    ...
    */
    case 'p':
      ...
      if(BTserial.available() >= 6)
      {
        ...
        BTserial.read(); // First byte of the page identifier can be ignored. It's always 0
        tempPage = BTserial.read();
        //currentPage = 1;
        offset1 = BTserial.read();
        offset2 = BTserial.read();
        valueOffset = word(offset2, offset1);
        length1 = BTserial.read();
        length2 = BTserial.read();
        length = word(length2, length1);
        for(int i = 0; i < length; i++)
        {
          BTserial.write( getPageValue(tempPage, valueOffset + i) );
        }
    ...
    case 'Q': // send code version
      //Serial.print(F("speeduino 202207"));
      BTserial.print(F("speeduino 202210-dev"));
      break;
    ...
    case 'r': //New format for the optimised OutputChannels
      ...
      if (BTserial.available() >= 6)
      {
        BTserial.read(); //Read the $tsCanId
        cmd = BTserial.read(); // read the command

        uint16_t offset, length;
        byte tmp;
        tmp = BTserial.read();
        offset = word(BTserial.read(), tmp);
        tmp = BTserial.read();
        length = word(BTserial.read(), tmp);

        serialStatusFlag = SERIAL_INACTIVE;

        if(cmd == 0x30) //Send output channels command 0x30 is 48dec
        {
          BTsendValues(offset, length, cmd, 0);
        }
        else
        {
          //No other r/ commands are supported in legacy mode
    ...
    case 'S': // send code version
      //Serial.print(F("Speeduino 2022.07"));
      BTserial.print(F("Speeduino 2022.10-dev"));
      currentStatus.secl = 0; //This is required in TS3 due to its stricter timings
      break;
    ...
    case 'T': //Send 256 tooth log entries to Tuner Studios tooth logger
      ...
      if(BTserial.available() >= 6)
      {
        ...
        if(currentStatus.toothLogEnabled == true) { BTsendToothLog_legacy(0); } //Sends tooth log values as ints
        else if (currentStatus.compositeLogEnabled == true) { BTsendCompositeLog_legacy(0); }
        serialStatusFlag = SERIAL_INACTIVE;
      }
      break;
    ...
    case 't': // receive new Calibration info. Command structure: "t", <tble_idx> <data array>.
      ...
      tableID = BTserial.read(); //Not currently used for anything

      BTreceiveCalibration(tableID); //Receive new values and store in memory
      writeCalibration(); //Store received values in EEPROM
    ...
    case 'W': // receive new VE obr constant at 'W'+<offset>+<newbyte>
      ...
      if (isMap())
      {
        if(BTserial.available() >= 3) // 1 additional byte is required on the MAP pages which are larger than 255 bytes
        {
          byte offset1, offset2;
          offset1 = BTserial.read();
          offset2 = BTserial.read();
          valueOffset = word(offset2, offset1);
          setPageValue(currentPage, valueOffset, BTserial.read());
          serialStatusFlag = SERIAL_INACTIVE;
        }
      }
      else
    ...
    case 'M':
      ...
      if(chunkPending == true)
      { 
        while( (BTserial.available() > 0) && (chunkComplete < chunkSize) )
        {
          setPageValue(currentPage, (valueOffset + chunkComplete), BTserial.read());
          chunkComplete++;
        }
        if(chunkComplete >= chunkSize) { serialStatusFlag = SERIAL_INACTIVE; chunkPending = false; }
      }
    ...
    case 'Z': //Totally non-standard testing function. Will be removed once calibration testing is completed. This function takes 1.5kb of program space! :S
    #ifndef SMALL_FLASH_MODE
      BTserial.println(F("Coolant"));
      for (int x = 0; x < 32; x++)
      {
        BTserial.print(cltCalibration_bins[x]);
        BTserial.print(", ");
        BTserial.println(cltCalibration_values[x]);
      }
      BTserial.println(F("Inlet temp"));
      for (int x = 0; x < 32; x++)
      {
        BTserial.print(iatCalibration_bins[x]);
        BTserial.print(", ");
        BTserial.println(iatCalibration_values[x]);
      }
      BTserial.println(F("O2"));
      for (int x = 0; x < 32; x++)
      {
        BTserial.print(o2Calibration_bins[x]);
        BTserial.print(", ");
        BTserial.println(o2Calibration_values[x]);
      }
      BTserial.println(F("WUE"));
      for (int x = 0; x < 10; x++)
      {
        BTserial.print(configPage4.wueBins[x]);
        BTserial.print(F(", "));
        BTserial.println(configPage2.wueValues[x]);
      }
      BTserial.flush();
    #endif
      break;
    ...
    case 'z': //Send 256 tooth log entries to a terminal emulator
      BTsendToothLog_legacy(0); //Sends tooth log values as chars
      break;
    ...
    case '`': //Custom 16u2 firmware is making its presence known
      ...
      if (BTserial.available() >= 1) {
        configPage4.bootloaderCaps = BTserial.read();
        serialStatusFlag = SERIAL_INACTIVE;
      }
      break;
    ...
    default:
      BTserial.println(F("Err: Unknown cmd"));
      serialStatusFlag = SERIAL_INACTIVE;
      break;
  }
}
...
void BTsendValues(uint16_t offset, uint16_t packetLength, byte cmd, byte portNum)
{
  serialStatusFlag = SERIAL_TRANSMIT_INPROGRESS_LEGACY;
  ...
  for(byte x=0; x<packetLength; x++)
  {
    if (portNum == 0) { BTserial.write(getTSLogEntry(offset+x)); }
    #if defined(CANSerial_AVAILABLE)
      else if (portNum == 3){ CANSerial.write(getTSLogEntry(offset+x)); }
    #endif

    //Check whether the tx buffer still has space
    if(BTserial.availableForWrite() < 1) 
    { 
      //tx buffer is full. Store the current state so it can be resumed later
      logItemsTransmitted = offset + x + 1;
      inProgressLength = packetLength - x - 1;
...
void BTsendValuesLegacy(void)
{
  uint16_t temp;
  int bytestosend = 114;
  
  bytestosend -= BTserial.write(currentStatus.secl>>8);
  bytestosend -= BTserial.write(currentStatus.secl);
  bytestosend -= BTserial.write(currentStatus.PW1>>8);
  bytestosend -= BTserial.write(currentStatus.PW1);
  bytestosend -= BTserial.write(currentStatus.PW2>>8);
  bytestosend -= BTserial.write(currentStatus.PW2);
  bytestosend -= BTserial.write(currentStatus.RPM>>8);
  bytestosend -= BTserial.write(currentStatus.RPM);
  
  temp = currentStatus.advance * 10;
  bytestosend -= BTserial.write(temp>>8);
  bytestosend -= BTserial.write(temp);
  
  bytestosend -= BTserial.write(currentStatus.nSquirts);
  bytestosend -= BTserial.write(currentStatus.engine);
  bytestosend -= BTserial.write(currentStatus.afrTarget);
  bytestosend -= BTserial.write(currentStatus.afrTarget); // send twice so afrtgt1 == afrtgt2
  bytestosend -= BTserial.write(99); // send dummy data as we don't have wbo2_en1
  bytestosend -= BTserial.write(99); // send dummy data as we don't have wbo2_en2
  ...
  bytestosend -= BTserial.write(currentStatus.battery10>>8);
  bytestosend -= BTserial.write(currentStatus.battery10);
  bytestosend -= BTserial.write(currentStatus.O2>>8);
  bytestosend -= BTserial.write(currentStatus.O2);
  bytestosend -= BTserial.write(currentStatus.O2_2>>8);
  bytestosend -= BTserial.write(currentStatus.O2_2);
  
  bytestosend -= BTserial.write(99); // knock
  bytestosend -= BTserial.write(99); // knock
  
  temp = currentStatus.egoCorrection * 10;
  bytestosend -= BTserial.write(temp>>8); // egocor1
  bytestosend -= BTserial.write(temp); // egocor1
  bytestosend -= BTserial.write(temp>>8); // egocor2
  bytestosend -= BTserial.write(temp); // egocor2
  
  temp = currentStatus.iatCorrection * 10;
  bytestosend -= BTserial.write(temp>>8); // aircor
  bytestosend -= BTserial.write(temp); // aircor
  
  temp = currentStatus.wueCorrection * 10;
  bytestosend -= BTserial.write(temp>>8); // warmcor
  bytestosend -= BTserial.write(temp); // warmcor
  
  bytestosend -= BTserial.write(99); // accelEnrich
  bytestosend -= BTserial.write(99); // accelEnrich
  bytestosend -= BTserial.write(99); // tpsFuelCut
  bytestosend -= BTserial.write(99); // tpsFuelCut
  bytestosend -= BTserial.write(99); // baroCorrection
  bytestosend -= BTserial.write(99); // baroCorrection
  
  temp = currentStatus.corrections * 10;
  bytestosend -= BTserial.write(temp>>8); // gammaEnrich
  bytestosend -= BTserial.write(temp); // gammaEnrich
  
  temp = currentStatus.VE * 10;
  bytestosend -= BTserial.write(temp>>8); // ve1
  bytestosend -= BTserial.write(temp); // ve1
  temp = currentStatus.VE2 * 10;
  bytestosend -= BTserial.write(temp>>8); // ve2
  bytestosend -= BTserial.write(temp); // ve2
  
  bytestosend -= BTserial.write(99); // iacstep
  bytestosend -= BTserial.write(99); // iacstep
  bytestosend -= BTserial.write(99); // cold_adv_deg
  bytestosend -= BTserial.write(99); // cold_adv_deg
  
  temp = currentStatus.tpsDOT * 10;
  bytestosend -= BTserial.write(temp>>8); // TPSdot
  bytestosend -= BTserial.write(temp); // TPSdot
  
  temp = currentStatus.mapDOT * 10;
  bytestosend -= BTserial.write(temp >> 8); // MAPdot
  bytestosend -= BTserial.write(temp); // MAPdot
  
  temp = currentStatus.dwell * 10;
  bytestosend -= BTserial.write(temp>>8); // dwell
  bytestosend -= BTserial.write(temp); // dwell
  
  bytestosend -= BTserial.write(99); // MAF
  bytestosend -= BTserial.write(99); // MAF
  bytestosend -= BTserial.write(currentStatus.fuelLoad*10); // fuelload
  bytestosend -= BTserial.write(99); // fuelcor
  bytestosend -= BTserial.write(99); // fuelcor
  bytestosend -= BTserial.write(99); // portStatus
...
namespace {
  ...
  void BTsend_raw_entity(const page_iterator_t &entity)
  {
    BTserial.write((byte *)entity.pData, entity.size);
  }
  ...
  inline void BTsend_table_values(table_value_iterator it)
  {
    while (!it.at_end())
    {
      auto row = *it;
      BTserial.write(&*row, row.size());
      ++it;
    }
  }
  ...
  inline void BTsend_table_axis(table_axis_iterator it)
  {
    const int16_byte *pConverter = table3d_axis_io::get_converter(it.get_domain());
    while (!it.at_end())
    {
      BTserial.write(pConverter->to_byte(*it));
      ++it;
    }
  }
  ...
  void BTsend_table_entity(const page_iterator_t &entity)
  {
    BTsend_table_values(rows_begin(entity));
    BTsend_table_axis(x_begin(entity));
    BTsend_table_axis(y_begin(entity));
  }
  ...
  void BTsend_entity(const page_iterator_t &entity)
  {
    switch (entity.type)
    {
    case Raw:
      return BTsend_raw_entity(entity);
      break;

    case Table:
      return BTsend_table_entity(entity);
      break;
...
void BTsendPage(void)
{
  page_iterator_t entity = page_begin(currentPage);

  while (entity.type!=End)
  {
    BTsend_entity(entity);
    entity = advance(entity);
  }
}
...
namespace {
  ...
  void BTserial_println_range(const byte *first, const byte *last)
  {
    while (first!=last)
    {
      BTserial.println(*first);
      ++first;
    }
  }
  ...
  void BTserial_println_range(const uint16_t *first, const uint16_t *last)
  {
    while (first!=last)
    {
      BTserial.println(*first);
  ...
  void BTserial_print_space_delimited(const byte *first, const byte *last)
  {
    while (first!=last)
    {
      BTserial.print(*first);// This displays the values horizontally on the screen
      BTserial.print(F(" "));
      ++first;
    }
    BTserial.println();
  }
  #define BTserial_print_space_delimited_array(array) BTserial_print_space_delimited(array, _end_range_address(array))
  ...
  void BTserial_print_prepadding(byte value)
  {
    if (value < 100)
    {
      BTserial.print(F(" "));
  ...
  void BTserial_print_prepadded_value(byte value)
  {
      BTserial_print_prepadding(value);
  ...
  void BTprint_row(const table_axis_iterator &y_it, table_row_iterator row)
  {
    BTserial_print_prepadded_value(table3d_axis_io::to_byte(y_it.get_domain(), *y_it));

    while (!row.at_end())
    {
      BTserial_print_prepadded_value(*row);
      ++row;
    }
    BTserial.println();
  }
  ...
  void BTprint_x_axis(const void *pTable, table_type_t key)
  {
    BTserial.print(F("    "));
    ...
    while(!x_it.at_end())
    {
      BTserial_print_prepadded_value(pConverter->to_byte(*x_it));
      ++x_it;
    }
  }
  ...
  void BTserial_print_3dtable(const void *pTable, table_type_t key)
  {
    auto y_it = y_begin(pTable, key);
    auto row_it = rows_begin(pTable, key);
    ...
    while (!row_it.at_end())
    {
      BTprint_row(y_it, *row_it);
      ++y_it;
      ++row_it;
    }

    BTprint_x_axis(pTable, key);
    BTserial.println();
  }
}
...
/** Send page as ASCII for debugging purposes over wireless serial connection.
 * Similar to sendPage(), however data is sent in human readable format. Sends page given in @ref currentPage.
 * 
 * This is used for testing only (Not used by TunerStudio) in order to see current map and config data without the need for TunerStudio. 
 */
void BTsendPageASCII(void)
{
  switch (currentPage)
  {
    case veMapPage:
      BTserial.println(F("\nVE Map"));
      BTserial_print_3dtable(&fuelTable, fuelTable.type_key);
      break;

    case veSetPage:
      BTserial.println(F("\nPg 2 Cfg"));
      // The following loop displays in human readable form of all byte values in config page 1 up to but not including the first array.
      BTserial_println_range((byte *)&configPage2, configPage2.wueValues);
      BTserial_print_space_delimited_array(configPage2.wueValues);
      // This displays all the byte values between the last array up to but not including the first unsigned int on config page 1
      BTserial_println_range(_end_range_byte_address(configPage2.wueValues), (byte*)&configPage2.injAng);
      // The following loop displays four unsigned ints
      BTserial_println_range(configPage2.injAng, configPage2.injAng + _countof(configPage2.injAng));
      // Following loop displays byte values between the unsigned ints
      BTserial_println_range(_end_range_byte_address(configPage2.injAng), (byte*)&configPage2.mapMax);
      BTserial.println(configPage2.mapMax);
      // Following loop displays remaining byte values of the page
      BTserial_println_range(&configPage2.fpPrime, (byte *)&configPage2 + sizeof(configPage2));
      break;

    case ignMapPage:
      BTserial.println(F("\nIgnition Map"));
      BTserial_print_3dtable(&ignitionTable, ignitionTable.type_key);
      break;

    case ignSetPage:
      BTserial.println(F("\nPg 4 Cfg"));
      BTserial.println(configPage4.triggerAngle);// configPage4.triggerAngle is an int so just display it without complication
      // Following loop displays byte values after that first int up to but not including the first array in config page 2
      BTserial_println_range((byte*)&configPage4.FixAng, configPage4.taeBins);
      BTserial_print_space_delimited_array(configPage4.taeBins);
      BTserial_print_space_delimited_array(configPage4.taeValues);
      BTserial_print_space_delimited_array(configPage4.wueBins);
      BTserial.println(configPage4.dwellLimit);// Little lonely byte stuck between two arrays. No complications just display it.
      BTserial_print_space_delimited_array(configPage4.dwellCorrectionValues);
      BTserial_println_range(_end_range_byte_address(configPage4.dwellCorrectionValues), (byte *)&configPage4 + sizeof(configPage4));
      break;

    case afrMapPage:
      BTserial.println(F("\nAFR Map"));
      BTserial_print_3dtable(&afrTable, afrTable.type_key);
      break;

    case afrSetPage:
      BTserial.println(F("\nPg 6 Config"));
      BTserial_println_range((byte *)&configPage6, configPage6.voltageCorrectionBins);
      BTserial_print_space_delimited_array(configPage6.voltageCorrectionBins);
      BTserial_print_space_delimited_array(configPage6.injVoltageCorrectionValues);
      BTserial_print_space_delimited_array(configPage6.airDenBins);
      BTserial_print_space_delimited_array(configPage6.airDenRates);
      BTserial_println_range(_end_range_byte_address(configPage6.airDenRates), configPage6.iacCLValues);
      BTserial_print_space_delimited_array(configPage6.iacCLValues);
      BTserial_print_space_delimited_array(configPage6.iacOLStepVal);
      BTserial_print_space_delimited_array(configPage6.iacOLPWMVal);
      BTserial_print_space_delimited_array(configPage6.iacBins);
      BTserial_print_space_delimited_array(configPage6.iacCrankSteps);
      BTserial_print_space_delimited_array(configPage6.iacCrankDuty);
      BTserial_print_space_delimited_array(configPage6.iacCrankBins);
      // Following loop is for remaining byte value of page
      BTserial_println_range(_end_range_byte_address(configPage6.iacCrankBins), (byte *)&configPage6 + sizeof(configPage6));
      break;

    case boostvvtPage:
      BTserial.println(F("\nBoost Map"));
      BTserial_print_3dtable(&boostTable, boostTable.type_key);
      BTserial.println(F("\nVVT Map"));
      BTserial_print_3dtable(&vvtTable, vvtTable.type_key);
      break;

    case seqFuelPage:
      BTserial.println(F("\nTrim 1 Table"));
      BTserial_print_3dtable(&trim1Table, trim1Table.type_key);
      break;

    case canbusPage:
      BTserial.println(F("\nPage 9 Cfg"));
      BTserial_println_range((byte *)&configPage9, (byte *)&configPage9 + sizeof(configPage9));
      break;

    case fuelMap2Page:
      BTserial.println(F("\n2nd Fuel Map"));
      BTserial_print_3dtable(&fuelTable2, fuelTable2.type_key);
      break;
   
    case ignMap2Page:
      BTserial.println(F("\n2nd Ignition Map"));
      BTserial_print_3dtable(&ignitionTable2, ignitionTable2.type_key);
      break;

    case boostvvtPage2:
      BTserial.println(F("\nBoost lookup table"));
      BTserial_print_3dtable(&boostTableLookupDuty, boostTableLookupDuty.type_key);
      break;
...
/** Processes an incoming stream of calibration data (for CLT, IAT or O2) from TunerStudio over wireless serial connection.
 * Result is store in EEPROM and memory.
 * 
 * @param tableID - calibration table to process. 0 = Coolant Sensor. 1 = IAT Sensor. 2 = O2 Sensor.
 */
void BTreceiveCalibration(byte tableID)
{
  void* pnt_TargetTable_values; //Pointer that will be used to point to the required target table values
  uint16_t* pnt_TargetTable_bins;   //Pointer that will be used to point to the required target table bins
  int OFFSET, DIVISION_FACTOR;
  ...
  else
  {
    ...
    for (uint16_t x = 0; x < 32; x++)
    {
      ...
      tempBuffer[0] = BTserial.read();
      tempBuffer[1] = BTserial.read();
...
void BTsendToothLog_legacy(byte startOffset)
{
  //We need TOOTH_LOG_SIZE number of records to send to TunerStudio. If there aren't that many in the buffer then we just return and wait for the next call
  if (BIT_CHECK(currentStatus.status1, BIT_STATUS1_TOOTHLOG1READY)) //Sanity check. Flagging system means this should always be true
  {
      ...
      for (int x = startOffset; x < TOOTH_LOG_SIZE; x++)
      {
        BTserial.write(toothHistory[x] >> 24);
        BTserial.write(toothHistory[x] >> 16);
        BTserial.write(toothHistory[x] >> 8);
        BTserial.write(toothHistory[x]);
      }
      BIT_CLEAR(currentStatus.status1, BIT_STATUS1_TOOTHLOG1READY);
      serialStatusFlag = SERIAL_INACTIVE; 
      toothHistoryIndex = 0;
  ...
  { 
    ...
    for(int x = 0; x < (4*TOOTH_LOG_SIZE); x++)
    {
      BTserial.write(static_cast<byte>(0x00)); //GCC9 fix
    }
    serialStatusFlag = SERIAL_INACTIVE;
  } 
}
...
void BTsendCompositeLog_legacy(byte startOffset)
{
  if (BIT_CHECK(currentStatus.status1, BIT_STATUS1_TOOTHLOG1READY)) //Sanity check. Flagging system means this should always be true
  {
      serialStatusFlag = SERIAL_TRANSMIT_COMPOSITE_INPROGRESS_LEGACY;
      ...
      for (int x = startOffset; x < TOOTH_LOG_SIZE; x++)
      {
        ...
        BTserial.write(inProgressCompositeTime >> 24);
        BTserial.write(inProgressCompositeTime >> 16);
        BTserial.write(inProgressCompositeTime >> 8);
        BTserial.write(inProgressCompositeTime);

        BTserial.write(compositeLogHistory[x]); //The status byte (Indicates the trigger edge, whether it was a pri/sec pulse, the sync status)
      }
      BIT_CLEAR(currentStatus.status1, BIT_STATUS1_TOOTHLOG1READY);
      toothHistoryIndex = 0;
      serialStatusFlag = SERIAL_INACTIVE; 
