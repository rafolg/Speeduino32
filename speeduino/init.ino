...
{   
    ...
    */
    ...
    {
      Serial.begin(115200);
      BIT_CLEAR(currentStatus.status4, BIT_STATUS4_WSER);
    }
    else if ((USBpinState == LOW) && (configPage9.enableWserial > 0))
    {
      ...
      char wifiSEP[] = ":";
      strcpy(wifiSettings, configPage15.SSID);
      strcat(wifiSettings, wifiSEP);
      strcat(wifiSettings, configPage15.WPWD);
      strcat(wifiSettings, wifiSEP);
      if (configPage15.wifiMode == 0) { strcat(wifiSettings, "0"); }        //AP
      else if (configPage15.wifiMode == 1) { strcat(wifiSettings, "1"); }   //STA
      strcat(wifiSettings, wifiSEP);
      switch (configPage15.wifiSecurity) {
        case 0:
          strcat(wifiSettings, "0"); // WPA3 Personal - Not all devices out there support this safest mode
          break;
        
        case 1:
          strcat(wifiSettings, "1"); // WPA2/WP3 Personal - Not all devices support this mode
          break;
        
        case 2:
          strcat(wifiSettings, "2"); // WPA2 Personal - Most compatible with devices today, not as safe as WPA3
          break;

        default:
          strcat(wifiSettings, "2");
          break;
      }
      strcat(wifiSettings, wifiSEP);
    ...
    */
    ...
    case IGN_MODE_SEQUENTIAL:
        ...
        knk1StartFunction = beginKnock1Measure;
        knk1EndFunction = endKnock1Measure;
        knk2StartFunction = beginKnock2Measure;
        knk2EndFunction = endKnock2Measure;
        knk3StartFunction = beginKnock3Measure;
        knk3EndFunction = endKnock3Measure;
        knk4StartFunction = beginKnock4Measure;
        knk4EndFunction = endKnock4Measure;
...
void setPinMapping(byte boardID)
{
  ...
  switch (boardID)
  {
    ...
    case 4:
      //Pin mappings as per modified v0.4-Teensy35 shield by Rafa
      pinInjector1 = 3;  //On T4.1 7&8 are usedl for Serial2
      pinInjector2 = 24; //teensy355 is 9 teensy41 is 53
      pinInjector3 = 25; //teensy35 is 10 teensy41 is 49
      pinInjector4 = 26; //teensy35 is 11 teensy41 is 50
      pinCoil1 = 31;
      pinCoil2 = 32;
      pinCoil3 = 29;
      pinCoil4 = 30;
      ...
      pinTPS = A2;
      pinMAP = A3;
      pinBaro = A16; //A21 Teensy (made-up-pin) A5 for fuelPressure and A6 for oilPressure
      pinIAT = A0;
      pinCLT = A1;
      pinO2 = A17; //This is A22 in teensy3.5
      pinBat = A4;
      pinDisplayReset = 51; //Just a made-up pin
      pinTachOut = 28;
      pinIdle1 = 5;
      pinIdle2 = 51; //Just a made-up pin as #4 is not connected in teensy-adapter
      pinBoost = 4; //On T4.1 7&8 Are used for Serial2
      pinVVT_1 = 6;
      pinVVT_2 = 51; //Just a made-up pin - not connected
      pinStepperDir = 51;
      pinStepperStep = 51;
      pinStepperEnable = 51;
      pinFan = 27;
      pinLaunch = 51; //Just a made-up pin - not connected
      pinFlex = 51;
      pinResetControl = 51; //Just a made-up pin - not connected
      pinVSS = 52; //Teensy35 is 12 Teensy41 is 52
      pinWMIEmpty = 39; //Made-up
      pinWMIIndicator = 38; //Made-up
      pinWMIEnabled = 1; //25 teensy is 42 on mega but not connected to anything on 0.4.4b
      pinSDEnable = 51; //Made-up
      pinTPIC8101_CS = 10; //This is just a test for the Teensy4.1 - Disable when going back to Teensy3.5
      pinTPIC8101_INT = 33; //Test for the Teensy4.1
      pinWMI = 0; // In T4.1 Development testing
      pinWserial = 21;
      /*
      MOSI1 = 26 / A12
      MISO1 = 39 / A15
...
#if(defined(CORE_TEENSY))
  if ( (configPage10.wmiPin != 0) && (configPage10.wmiPin < BOARD_MAX_IO_PINS)) { pinWMI = pinTranslate(configPage10.wmiPin); }
#endif
...
#if(defined(CORE_TEENSY))
  pinMode(pinWMI, OUTPUT);
  //pinMode(0, INPUT_PULLDOWN); //Couldnt get it to work
  //pinMode(txPin, OUTPUT); Couldnt get it to work
#endif
...
void initialiseTriggers(void)
{
  ...
  switch (configPage4.TrigPattern)
  {
    case DECODER_MISSING_TOOTH:
      ...
      //triggerSetEndTeethK = triggerSetEndTeethK_missingTooth;
